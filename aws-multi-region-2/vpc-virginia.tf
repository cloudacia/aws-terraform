...
####################################################
resource "aws_eip" "ip_nat01_virginia" {
  vpc = true

  tags = {
    Name = "NAT GATEWAY VIRGINIA"
  }
...
# VPC NAT Gateway                                  #
####################################################
resource "aws_nat_gateway" "gw01_vpc_virginia" {
  allocation_id     = aws_eip.ip_nat01_virginia.id
...
#############################################################
resource "aws_route" "nat_gateway01_vpc_virginia" {
  route_table_id         = aws_route_table.rt02_vpc_virginia.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_nat_gateway.gw01_vpc_virginia.id
